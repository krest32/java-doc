...
或者双击startup.cmd运行文件。
访问：http://localhost:8848/nacos用户名密码：nacos/nacos
~~~
...
1. 单机模式的时候，Nacos使用嵌入式的数据库实现啊数据的存储，但是如果想要使用外部的mysql存储Nacos数据，需要以下步骤：
2. 安装数据库：5.6.5+，mysql8以下
3. 初始化数据库建立数据库nacos_config，然后执行Sql语句
4. 然后修改nacos中的配置，改变成外部链接Mysql
5. application.properties

~~~
spring.datasource.platform=mysqldb.
num=1db.
url.0=jdbc:mysql://127.0.0.1:3306/nacos_configcharacterEncoding=utf8&connectTimeout=1000&socketTimeout=3000&autoReconnect=truedb.
user=rootdb.
password=123456
~~~
...
1. 新增配置信息
   ...
   ~~~yml
   data id：Group：DEFAULT_GROUP配置格式：YAML配置内容：common:			config1:something
   ~~~
...
public class SimpleDemo {    
    public static void main(String[] args) throws NacosException {        
        ...
        //获取配置        
        //需要三个参数 Dataid、group、超时时间（因为是通过网络获取，所以如果获取不成功要及时停止）       
...
通过Namespace、Group、DataID获得配置信息

DATAID

​		在系统中，**一个配置文件就是一个配置集**，一个配置集包含了系统的各种配置信息，所以配置集应该有一个有意义的名称。配置集的ID就是DataID

配置项

​		配置集中包含的一个个的**配置内容就是配置项**

配置分组GROUP

​		对配置集进行分组，可以用来**区分不用的项目和应用**，例如：学生管理系统就可以定义Group为：STUDENT_GROUP

Namespace

​		可以用于对于**不同环境的配置隔离**，例如开发环境、测试环境、测试环境；针对不同的开发人员使用同一个nacos管理各自的配置，通过不同的namespace进行配置隔离管理，默认是public的环境。
...
​		克隆其他namespace的配置到另一个namespace中
...
1. 当远程端，配置发生改变时，Nacos会通知到Java后台

2. java程序

~~~JAVA
//监听配置服务，当配置发生改变的时候，就及时得到配置       
...
>
>    ~~~xml
>    service1Namespace devDataID Service1.yamlGroup:TEST_GROUP
>    配置格式：YAML配置内容：common:	name: Service1_config
>    ~~~
>
> 2. 新建工程添加依赖
>
...
> 1. 新建微服务
>
> 2. 将nocas的配置添加到bootstrap.yml中，因为它的优先级是大于application.yml的，会被优先加载
>
> 3. （常用）一个微服务可以对应多个配置文件，但是默认的Group是DEFAULT_GROUP，也可以支持改变GROUP，==但是所有对应的多个DATAID不支持动态刷新==，如果想要支持动态刷新，需要下面的语句
>
>    ~~~xml
>    ext-config[0]：	data-id: ext-config-common01.properties	group: REFRESH_GROUP	refresh: true
>    ~~~
>
> 4. 程序不仅仅读取了service1.yml的配置文件内容，还读取了ext-config-common01.properties的配置内容
>
> 5. （不常用）然也可以才用自定义共享DataID，但是**仅承认DEFAULT_GROUP**的配置文件
>
>    ~~~yml
>    spring:	
>    	cloud:		
>    		nacos:			
>    			config:				
>    				shared-dataids: ext-config-common01.properties,ext-config-common02.properties						     refreshable-dataid: ext-config-common01.properties
>    ~~~

#### 5.扩展优先级

> 默认的优先级最高>自定义扩展id（n的值越大，优先级越高）>共享id
>
> ~~~java
> @RestController
...
>         String name = applicationContext.getEnvironment().getProperty("common.fullname");
...
>
> 其中Nacos支持的功能是最全面的。长远来看，还会支持SpringCloud+K8s的组合，而且计划实现Service Mesh

### 综合集成架构演示

#### 1. 简介

> ​		Spring cloud 是一个较为全面的微服务框架集合，==网上通常将SpringCloud和Dubbo做对比，但是这种做法不恰当==，前者是一个较为完整的架构方案，而后者只是服务治理和RPC实现方案
>
> ​		==Dubbo的性能很好==，Dubbo在国内有非常大的用户群体，但是周边的设施和组件相对不太完善，直到SpringCloud ALibaba的出现，才将Dubbo和Spring Cloud统一起来。

####　2. （重点）实现Alibaba+cloud架构
...
> Spring Cloud 常见的继承方式是==Feign+Rinnon==（客户端负载均衡）技术来事项服务间的远程调用和负载均衡，用来处理高并发、缓解网络压力和进行网络扩容的重要手段之一。它分为服务端和客户端的负载均衡
>
> #### 1、在service模块配置pom
>
> 配置Nacos客户端的pom依赖
...
默认为所属工程配置spring.application.name 的值（即：nacos-provider），
也可以通过配置项 spring.cloud.nacos.config.prefix来配置。
- spring.profiles.active=dev 即为当前环境对应的 profile。 
注意：当 spring.profiles.active 为空时，对应的连接符 - 也将不存在，
dataId 的拼接格式变成 ${prefix}.${file-extension}- file-exetension 为配置内容的数据格式，
...
spring.cloud.nacos.config.ext-config[0].refresh=truespring.cloud.nacos.config.ext-config[1].data-id=log.properties
...
spring.cloud.nacos.config.ext-config[0].data-id=actuator.properties
spring.cloud.nacos.config.ext-config[0].group=DEFAULT_GROUP
spring.cloud.nacos.config.ext-config[0].refresh=truespring.cloud.nacos.config.shared-dataids=log.properties
...
>    ```
>    
>    4. 在service-edu的client包里面创建熔断器的实现类
>    
>    ```java
>    @Component
...
#配置service-edu服务
spring.cloud.gateway.routes[1].id=service-eduspring.cloud.gateway.routes[1].uri=lb://service-edu
spring.cloud.gateway.routes[1].predicates= Path=/eduservice/**
#配置service-ucenter服务
spring.cloud.gateway.routes[2].id=service-ucenterspring.cloud.gateway.routes[2].uri=lb://service-ucenter
spring.cloud.gateway.routes[2].predicates= Path=/ucenterservice/**
#配置service-ucenter服务
spring.cloud.gateway.routes[3].id=service-cmsspring.cloud.gateway.routes[3].uri=lb://service-cms
...
spring.cloud.gateway.routes[9].id=service-vodspring.cloud.gateway.routes[9].uri=lb://service-vod
...
![img](C:/Users/Krest/Documents/My%20Knowledge/temp/38822f0c-1f72-42fd-999f-84757b380f5f/128/index_files/25718906-9518-4e35-9848-d2f10c079389.png)

 

```java
//解决跨域问题，不用再添加Crossorigin
...
![img](C:/Users/Krest/Documents/My%20Knowledge/temp/38822f0c-1f72-42fd-999f-84757b380f5f/128/index_files/fb0f4296-96e9-4141-a1f5-52e69f1929b8.png)
