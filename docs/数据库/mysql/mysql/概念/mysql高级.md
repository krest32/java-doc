...
1. 仅支持InnoDB表，并且每一张表都要有一个主键，用于做write set的冲突检测
2. 必须打开GTID特性，二进制日志格式必须设置为Row，用于选主和writeset；主从状态信息存于表中
3. MGR不支持大事务，事务大小最好不要超过143MB。当事务过大，无法在5S的时间内通过网络在组员之间复制消息，则可能会怀疑成员失败了，然后将其驱逐出局
4. 目前MGR集群最多支持9个节点
5. **不支持外键**与save point特性，无法从全局间的约束检测与部分事务回滚
6. 二进制日志不支持Binlong Event CheckSum（Binlog 检查）

**使用场景：**

1. 金融交易、数据存储，对主从一致性要求比较高的场景
2. 核心数据总量未过亿
3. 读多写少的场景，比如互联网电商
