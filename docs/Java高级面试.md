...
+ 分片中间件：MayCat、Sharding Sphere，（一般来说已经使用绝大多数企业了）
  + 适合10亿级别的数量总集的大型应用
...
2. Redis对比其他？为什么高性能？
3. Redis的数据结构有哪些？
4. 关于Redis的持久化机制的理解？（高性能和数据0丢失方面，往往只能够满足一个条件）
5. Redis的过去Key删除策略？
6. Redis的内存淘汰策略？ 8种
7. 常见的缓存问题？缓存雪崩、缓存穿透、缓存击穿、双写一致性
8. Redis 的慢查询记录？
9. Redis分布式锁的使用？
10. Redis 对于高并发高可用的实现
...
+ 将所有任务放入到数组中，然后轮训
+ 排序方案，维持最小堆
+ 时间轮方案，如果任务过多导致时间刻度增加，还可以使用多级时间轮的方案
...
+ 避免帐号密码泄漏，一个授权码，可以完成授权工作，同时还可以设置有效期
