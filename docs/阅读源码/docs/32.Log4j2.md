...
1. 核心入口是LogManager
2. 通过LonManager获取得到LongContext，这里对于Context的创建会通过LogContextFactory
3. Context的创建需要阅读配置文件，这个时候，会有三种形式：xml、yml、properties
4. 会经过XmlConfiguration
5. 成功创建Context后，会根据Class创建对应的Logger

## 同步日志运行流程

1. AbstractLogger.info():模板方法，定义logger的工作
2. Logger.isEnabled():判断是否有权限
3. Logger.filter():判断当前的Logger的日志级别是否大于输出的日志级别。
4. AbstractLogger.logMessage()
5. AbstractMessage.newMessage()->new SimpleMessage()
6. LogConfig.log():日志打印的主要逻辑
7. LogEventFactory:DefaultLogEventFactory.createLogEvent():将日志包装成为一个logEvent
8. LogConfig.callAppends(): 获取对应的 Appender
9. AppendContrl.append(): 调用 appender 打印日志
10. AbstractOutputStreamAppend.wirte(): 打印到控制台
