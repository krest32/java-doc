...
3. 谈谈你对于面向对象的理解？
   ...
   + 什么是API和SPI？
   + 抽象类和抽象接口的区别？
   + 两个不同对象的HashCode也可能会相同，所以才会有equal方法，当Hashcode相同时，就是用Equal进行比较
...
10. 谈谈你对并发编程的理解？
    ...
    + 对线程安全的理解？（针对一段代码来说，它是否是安全的）
    + 线程安全的处理方式通常有哪些？
    + 线程安全方式之间的对比？synchronized 与 lock
    + Synchronized锁升级的过程？
    + ReentrantLock 的理解？
      + 概念、特性、实现原理（如何实现  可重入锁?）
    + 创建线程的集中方式？
    + 线程池相关
      + 线程池的七大参数？
      + 设置线程池的线程设置大小
    ...
    + 并发容器有哪些？
    + 并发工具类 countDownLatch 和 Semphere
      + countDownLatch ：底层实现，使用 await 会阻塞，等待所有事项一起完成再
      + Semphere：相当于是一个许可，有几个停车位
...
21. 谈谈你对于Spring Boot的理解？

    + 什么是Spring Boot？
    + Spring Boot自动装配的原理是怎么样的？
    + 如何自定义Starter？

22. 谈谈你对于Spring Cloud的理解？
