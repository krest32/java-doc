...
4. 为什么说Java语言编译与解释共存？
5. `Java`程序执行的顺序是什么样的？
6. `JDK 8` 的主要特性有哪些呀？（JVM 元空间、Stream流、CompletableFuture、lambda、LocalTime LocalDate包）
...
16. 对于 `final` 关键字的理解？
17. 对于 `static` 关键字的理解？
18. `try... catch.... finally` 中 ` finally`  使用 `return` 为什么是禁止的 ？
19. 谈谈你对于枚举的理解？
20. `this` 和`super`之间的区别是什么？
21. `==` 和 `equal` 之间的区别？
22. 对象比较时，为什么要重写 `Equal()`  方法后，一定要重写 `hashcode()` 方法？
23. 什么是面向对象开发，什么是面向过程，二者对比有什么区别？
24. 面向对象开发的基本原则是什么？（六大原则）
25. 为什么要有基本数据的包装类？
26. `Integer` 缓存池有了解过么 ？
27. `new Integer` 和 `Integer.valueOf()` 之间的区别是什么？一般在项目中哪一个？
28. 为什么要使用缓存池？缓存池在什么位置？采用了使用了什么设计模式？
29. 对象实例和对象引用之间的区别是什么？
30. 成员变量和局部变量之间的区别是什么？
31. 在调用子类构造方法之前会先调用父类没有参数的构造方法，其目的是？
32. 什么是内部类？为什么要有内部类？
33. 局部内部类的分类有哪些？
34. 构造方法是否可以被重写？可以被重载么？
35. 接口和抽象类的区别？
36. `Java` 使用的值传递还是引用传递？二者有什么区别
37. 什么是 `I/O` ？
38. `I/O`流主 要有哪些分类？
39. 谈谈你对于IO有哪些认识？
40. `BIO`、 `NIO`、`AIO` 是什么？有什么区别？
41. `BIO`、`NIO`的使用场景？
42. 字符流与字节流有什么区别，用到了什么设计模式？
    1. 字符流拥有缓存区
    2. 字符流方便我们操作数据 
43. 多路复用 `I/O` 模型的概念？它有什么局限性？
44. 谈谈你对Linux的五种IO模型分别是什么？
    1. Linux的多路复用IO有哪些种类？

45. 序列化有什么用？`serivalVersionUUID` 作用？ `transient` 的作用？
46. 被`transient `修饰变量的初始值会是什么？
47. `Java` 序列化的缺点有哪些？
48. 分布式系统序列化的发展历史？
    1. SOP协议
    2. Restful + Json
...
51. 获取反射的3种方法？
52. 如何提高反射的效率 ？
53. 项目中你们的日期格式转化一般是怎么做的？（比如字符串转化为Date）
54. 为什么要使用日志框架？
55. 日志的输出级别有哪些？
56. 最近关于 `log4j2` 的`bug`有了解过么？漏洞补救措施，漏洞解决措施？
57. 谈谈你对`Java` 异常的理解？
58. `Java` 异常的分类有哪些？
59. `JVM` 时如何处理异常的？
60. 什么时受检时异常？
...
63. 什么是泛型？
64. 为什么要使用泛型？
65. 常用通配符代表的含义 (`T` `?` `K V` `E`)?
66. 什么是类型擦除？
67. 如何在`List<Integer>`中插入字符串或者浮点数？   
