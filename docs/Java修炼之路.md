...
互联网企业野蛮生长的时代已经过去，日益复杂的国际形式也给经济发展带来了新的挑战。中国企业当下的核心目标从低端制造业中脱离出来，完成产业升级。我个人从事后端开发工作，未来几年信息产业如何发展与我自身也是息息相关的，接下来我会看好以下行业：

1. 第一个就是人工智能，当下热门的有ChatGPT、汽车的自动驾驶、全屋家具智能化等；
2. 第二个是大数据领域，它的应用范围比较广，比如：金融、医疗、气象等；
3. 第三个是IT为各个行业进行赋能，比如：智慧校园、智慧水电等

目前大数据和人工智能会是未来国家战略性的产业。前途一片光明，但是它们的入行门槛比较高，招聘的学历一般在硕士及以上。

再说传统前后端岗位，互联网发展这么多年，it技术相对来说很成熟了，你在工作中遇到的问题，其中99%都会有现成解决方案。it行业目前来说，不仅仅卷技术，一些公司，它在招聘的时候，直接就会限制年龄和学历。未来几年，这个行业的竞争压力恐怕会持续增加。那么我目前作为一个后端开发人员，能做的就是不断提升和完善自己的技术，以便适应当下的行业发展。



## 前后端工作本质

这些年工作下来，我对于编程的理解就比较简单，概述一下就是：我们不生产数据，我们只是数据的搬运工。

+ 前端工作的核心：就是将数据展示给用户。
+ 后端工作的核心：就是在浏览器与服务器之间，完成数据的存储和传输工作

这个过程类似于我们在电脑中打开一个word文档，然后进行一系列的新增、修改、删除等操作。只不过通过编程我们可以实现在本地电脑对远端服务器的数据的进行类似操作。

因为要展示数据，所以需要浏览器，比如谷歌浏览器或者火狐浏览器等。

因为实现交互数据的逻辑，所以我们需要使用相关的编程语言，比如JavaScript、Java、Golang、Python、C、C++等

因为要存储数据，所以会用到数据库，比如 Mysql、Orcal等

IT的工作并没有很多人想的那么高大上，它工作的本质其实就是对数据的一些列操作，只不过经过时间的发展, 数据处理的方式已经发生了天翻地覆的变化

## 后端除了增删改查还有什么？

后端开发的工作内容：（这里主要针对业务开发人员）

+ 首先，最基本的是通过增删改查的方式，完成对数据的处理和交互工作，这也是大多数程序员日常70%以上的工作内容。这个过程会优先保证程序实现需求。
+ 其次，功能性实现后，后端要保证系统性能达标，比如最简单的，某个接口的响应时长规定在多少毫秒以内，那这个时候可能会涉及一些性能优化的东西，比如：数据库设计与Sql使用是否合理、接口内部的请求能不能使用多线程异步的形式
+ 然后要考虑，系统的功能会不会随时间追加增加呢？因为当一个系统很复杂的时候，我们还需要对系统进行拆分，这个时候，会引进微服务体系架构，这个时候就会涉及到系统重构。
+ 最后其实就是被人们说烂的两个东西：高可用和高并发。这是一个量变到质变的过程，常见的问题有：系统应对的流量是多少？是否要保证系统高可用？机器的配置如何等

可以总结一下，就是增删改查只是后端开发最基本的工作内容，如果想要职业发展道路更加长远，要走的路其实还有很长。

然后还有一些题外话补充：技术的发展日新月异，同样的功能，也许在未来几年会有更好的实现方式，这也是后端工作人员最大的挑战和乐趣之一。
...
概述：不同的语言存在，目的就是为了处理一些特定的问题，没于任何一门编程语言是万能的，任何一门编程语言都可能会没落。

1. JavaScript：简称JS，是前端最流行的语言，完成和后端接口的数据交互，实现页面效果和数据的动态展示
2. TypeScript，简称TS，是JS的超集，能够完全的兼容JS，并在JS的基础上进行功能扩展，目前也是前端的当前的热门语言
3. Python通常会用来作为脚本类语言。因为类库繁多，它也是AI领域的大热门，缺点是性能比较差
4. Java的性能不错，同时支持网络编程和并发编程，主要用于Web后端开发，可以构建大型应用系统、游戏服务器。缺点是编译慢，运行时需要的内存比较高
5. C、C++ ，流行了几十年的编程语言，可以直接调用系统资源，通常用来编写系统核心组件，但是上手难度比较高，
6. Go类似于C C++，但是比更加简单，同时天然支持高并发，目前Go主要有两种方向
   1. 基本应用组件开发：K8s、istio
   2. 核心高并发业务，
7. R数据分析、科学计算，这个语言的安全性很高，难度也比较大，会用于比较小的项目



### 如何评价某一个技术（好坏）？

个人感觉，技术没有好坏之分，只有适不适合这一种说法。一个技术的存在往往都是为了解决一种特定的问题。对于解决同一个问题的新旧技术来说，我的看法是新技术转可能是站在了旧技术的肩膀上，才能实现进阶和跨越。新技术的进步，永远需要老技术的沉淀。

就比如Go语言，如果Go语言之前没有C或者C++这两个语言，我并不觉得Go语言今天能够被开发出来。同样如果没有Go语言，我们今天可能就无法使用Docker、K8s这样的工具。

再比如使用场景问题，服务端语言，Java称霸了20多年，然而在人工智能方面，Python依旧是王者。

我当然可以使用Java实现人工职能，使用Python构建服务器，但是没有这种必要，好么？不要总想着让一个打篮球的运动员去练体操。

IT界有个笑话，写C的看不起写C++的，写C++的看不起写Java的，写Java的看不起写Python的，写Python看不起写PHP的，但PHP却是世界上最好的编程语言。

总结一下：脱离时代和使用场景去评价任何一门技术，都是在耍流氓。
...
8. 为什么要固守使用JDK8？对比JDK11和JDK17?
9. 可以说一下对于JDk19的新特性？
10. 可以说一下对于JDK21的新特性？
11. Java的八种基本数据类型有哪些？
12. 最有效的计算2乘以8？
13. switch能否用在String上？那么Long呢？为什么不支持Long？
14. null是什么？
15. Java语言采用的编码方案？
16. 什么是不可变类？常见的不可变类有哪些？
17. 什么是数组？数组有哪些特点？
18. 对于final关键字的理解？
19. 对于static关键字的理解？
20. this和super之间的区别是什么？
21. ==和equal之间的区别？
22. 对象比较时，为什么要重写Equal()方法后，一定要重写hashcode()方法？
23. String为什么会比StringBuilder慢？
24. String为什么要设计为不可变类？
    1. 线程安全
    2. 哈希值唯一，用作Map的key可以避免重新计算hash值
    3. 提高常量池的可用性，多个引用可以指向同一个字符串，大大节省了内存
25. JDK9为何要将*String*的底层实现由*char*[\]改成了*byte*[]？
...
2. 为什么要有基本数据的包装类？
3. Integer缓存池有了解过么？
4. new Integer()和Integer.valueOf()之间的区别是什么？一般在项目中哪一个？
5. Java的整数包装类采用了使用了什么设计模式？
6. 接口和抽象类的区别？
7. 谈谈API和SPI之间的区别？
8. 成员变量和局部变量之间的区别是什么？
...
15. Java使用的值传递还是引用传递？二者有什么区别
16. Java中常用的包有哪些？
    1. `java.io`：这里面是所有输入输出有关的类，比如文件操作等；
    2. `java.lang`：这个是系统的基础类；
...
7. 谈谈你对Linux的五种IO模型分别是什么？
   1. Linux的多路复用IO有哪几种？
   1. Linux的多路复用IO有哪些种类？epoll为什么性能高？

      1. select和poll都是基于轮询的方式
   2. epoll基于事件驱动的方式，性能比前两者更高一些
...
4. 常用的集合有哪些？
5. List、Set、Map之间有什么区别？
6. 哪些集合类是线程安全的？
7. 关于FastFail机制是什么？它有什么作用 ？
8. 怎么确保一个集合不能被修改？
9. 谈谈你对于iterator的理解？
10. 如何边遍历边移除 Collection 中的元素？
11. ArrayList与LinkedList之间的区别?
12. ArrayList的插入删除性能一定会比LinkedList慢么？
13. ArrayList和Vector的区别是什么？
14. 为什么ArrayList的elementData加上transient修饰？
    + 重写了writeObject方法，只序列化已存入的元素，这样加快了序列化的速度
15. List和Set之间的区别是什么？
16. Queue是什么？
17. BlockingQueue是什么？
...
20. 解决哈希冲入都有哪些方式呀？（四种）
21. 谈谈HashMap的原理？
22. 说一下HashMap的几个参数：负载因子、初试容量？
23. 为什么不直接使用HashCode作为table下标？
24. HashMap的扩容流程？
25. 为什么HashMap扩容时候，是原本大小的2倍？
26. 为什么负载因子是0.75？
27. HashMap在扩容上有哪些优化？
    + 1.7以前都需要重新计算哈希，
...
12. STW一般发生在程序的什么时候 ？
13. G1的算法原理是什么？相比分代算法的优势？对比CMS又有什么样的特点？
14. 了解过ZGC么？（专门针对超大数据量的垃圾回收器）
15. 什么是GC的吞吐量？（一次尽可能多的回收垃圾）
16. 什么是GC的低延迟？（每次回收垃圾的时间都很短）
...
4. 你常用的 Linux 指令有哪些？
   1. 文件管理：`mkdir`、`mv`、`rm`等 
   2. 查看或者编辑文件：`cat`、`tail 、`vim`等
   3. 压缩：`tar`、`unzip`等
   4. 系统操作：`top`、`systemlctl`、`ps`等
   5. 用户管理相关指令
5. 程序一般安装和使用流程
   1. 新建用户
   2. 添加文件夹，设置文件归属
...
不同的数据库其实主要是为了满足不同的使用场景，所以，技术没有好坏，重要的是找到他们真正适合的地方

1. 市面上常见数据库的区别？
   1. MySql-> 最流行的免费的关系型数据库
   2. PostgreSql-> 最先进的，功能很完善，有很多尝鲜的功能
   ...
   9. ClickHouse-> 大数据列式存储，主要用作大数据场景进行**数据报表分析**使用，但是不支持事务，且单次存储性能低下，不适合作为OLTP数据库
...
6. ACID是靠什么保证的？
7. Mysql-两阶段提交？
   1. 先提交Redo Log状态为prepare
   2. 写入到数据库中
   3. 写入binLog，更新RedoLog的状态为commit
8. 什么是脏读、幻读、不可重复读？
9. 谈谈你对MySql事务的隔离级别的理解？
10. 当前读和快照读？如何实现当前读（共享锁或排它锁）？
11. 隔离级别和锁之间的关系？
12. Mysql锁：行锁、表锁、页锁、元数据锁？
13. InnoDB行锁的分类有哪些？
14. InnoDB的行锁模式？
15. 为什么要避免间隙锁？
16. 什么情况下行锁升级表锁？
...
6. 如何优化大分页查询？explain 字段的作用，还有常看哪些字段？
...
2. `Redis`的内存淘汰策略有哪些？
   ...
   1. Redis回内存收使用的是什么算法？有8种
   2. `LRU` (Least Recently Used) 算法 - 最近最少使用，`Redis 3.0`中的`LRU`算法，其核心思想是"如果数据最近被访问过，那么将来被访问的几率也更高"所以当新的数据进来时我们可以优先把这些数据替换掉。是淘汰最长时间没有被使用的页面。
   3. `LFU` (Least Frequently Used) 算法 - 最不经常使用，`Redis 4.0`中新增的`LFU`算法，LFU是淘汰一段时间内，使用次数最少的页面。其核心思想是"如果数据过去被访问多次，那么将来被访问的频率也更高"

3. LFU对比LRU的优缺点？ 

   对比方向：命中率、复杂度、代价

4. Redis如何做内存优化？（内存可能只有20G，但需要你放入原有30G，同时Redis本身的性能）
...
1. 什么是`ORM`？
2. 为什么要使用`ORM`？
3. `Mybatis`对比`JPA`，分别适用于什么样的使用场景？
...
4. 为什么要是用 Spring？
5. Spring的使用场景？
6. Spring的主要组成模块（起码说出6个）？
7. Spring中使用到的设计模式有哪些？
...
大文件上传，如果这个服务的使用量上来了，那么就会导致内存被撑爆，因为上传的文件可能会比较大大，导致GC无法回收内存
...
3. 缓存自动扩容：结合云处理器，高峰时刻进行动态扩容，他缓存使用的是：Pivotal Gemfire
...
[解析](https://witerk.top/#/./docs/%E5%9F%BA%E7%A1%80%E9%83%A8%E5%88%86/%E8%99%9A%E6%8B%9F%E6%9C%BA/JVM)

### MySQL数据库cpu飙升到100%？

当 cpu 飙升到500%时，先用操作系统命令 top 命令观察是不是 mysqld 占用导致的，如果不是，找出占用高的进程，并进行相关处理。如果是 mysqld 造成的， show processlist，看看里面跑的session情况，是不是有消耗资源的 sql 在运行。找出消耗高的sql，看看执行计划是否准确，index是否缺失，或者实在是数据量太大造成。

+ 一般来说，肯定要 kill 掉这些线程(同时观察 cpu 使用率是否下降)，等进行相应的调整(比如说加索引、改 sql、改内存参数)之后，再重新跑这些SQL。
...
+ 假设1：tomcat 默认的最大线程数是200，如果单个接口的调用时间为1s，那么这个接口的吞吐量在200/s，
+ 假设2：tomcat 默认的最大线程数是200，如果单个接口的调用时间为0.5s，那么这个接口的吞吐量在400/s，

设置经验值：1核2G为200；4核8G为800，但是需要注意，线程数量越多，就意味着系统的稳定性越差。所以线程数的设置是一个多方考量的结果
...
1亿红包，3000万人抢，红包数量1000万个，TPS预计百万，如何设计
...
+ 现在架构：Es多集群，对于日志的优先级进行拆分
  ...
  + 冷热分离：Hot节点使用SSD高配，支持实时数据，Cold节点，存储历史数据
