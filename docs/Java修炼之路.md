...
4. `MyBatis` 的工作原理（查询数据的原理）？
5. `#{}`  与 `?{}` 之间有什么区别？
6. `Mybatis`懒加载？为什么要懒加载？懒加载会带来什么问题？
7. N+1的问题如何解决？
8. `Mybatis` 的一级缓存 与 二级缓存 有什么理解 ？作用域有什么区别？（`session`、`nameSpace<cache>标签`）
9. `Mybatis`插件实现原理？
