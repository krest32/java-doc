...
总结：未来产业，个人会更加看好大数据，它也是国家战略性的产业，所以是重中之重，同时计算机也在为传统行业进行数据化赋能，不过大数据行业仍然依托于业务的电子信息化，未来随着科技与业务的发展，技术领域会越来越细分化，而每个时期都会有不同的科技或产业发展重点。
...
谈谈你对于 Java 的理解？为什么是`Java`而不是其他？有什么优势？有什么缺点？
...
+ 编译语言直接将程序转换为机器代码。结果，这些语言比解释器语言快得多。解释器花费很少的时间来分析源代码。但是，执行该过程的总时间要慢得多。与诸如C或C ++的编译语言相比，这使Interpreted编程语言的执行非常慢。这就是为什么Python比C ++和Java慢的原因。Java既不是编译语言也不是解释语言。它位于中间。Java是与平台无关的语言，这使其在所有编程语言中都非常流行。字节码使其成为一种与平台无关的语言。这就是Java的优势。
...
8. 谈谈你对Linux的五种IO模型分别是什么？
   1. Linux的同步同步非阻塞IO，有哪些youquedian 
...
8. 如果 finally 中发生了异常，会怎么办？（try 的语句会执行，finally的语句会抛出异常）
...
1. 谈谈你对 `java.util` 集合框架的理解？（都有哪些接口和类，集合框架的五大部分）
2. 集合底层的数据结构是什么样子的？
...
16. 解决哈希冲入都有哪些方式呀？（四种）
17. 谈谈 `HashMap` 的原理？
18. 说一下`HashMap`的几个参数：负载因子、初试容量？
19. 为什么不之间使用 `HashCode` 作为 table 下标？
20. `HashMap` 的 扩容流程？
21. 为什么`HashMap`扩容时候，是原本大小的2倍？
22. `HashMap`在扩容上有哪些优化？
    + 1.7以前都需要重新计算哈希，
    + 1.8以后，借助2备扩容机值，不需要重新计算哈希值，低位元素在原来位置，高位元素在原来位置+原来容量的大小

23. `HashMap`中的链表转化红黑树需要哪两个条件？
24. 为什么`String`和`Integer`适合作为`Map`的`Key`?
25. 可以使用`Object`对象作为`key`么？
26. 关于 `TreeMap` 的理解说一下？新元素插入需要提起准备什么？
27. HashMap的那些操作会导致线程不安全？
...
13. `runnable` 接口与`callable`接口之间的区别？

14. `run()` 和`start()`之间的区别？

15. 同一个线程可以两次调用`start()`方法么？

16. 什么是`Callable`、`Future`、`FutureTask`？ 

17. `Future`、`FutureTask` 与 `Completefutere` 的区别？
...
22. 常用的线程操作方法 `sleep` 、`wait`、`notify`、`notifyAll`，简单聊一下他们的使用？

23. 为什么`wait`、`notify`、`notifyAll`是 `Object`方法？

24. 关于 `sleep() `方法 与 `yeild()` 方法有什么区别？

25. 什么时候使用`join()`方法

26. 如何停止一个正在运行的线程？`interrupted()` 和 `stop()`

27. 如何实现多线程之间的通讯和协作（两种方案）？
...
1. 市面上常见数据库的区别？
   ...
   2. `PostgreSQL`-> 最先进的
   3. `Oracle` -> 最完善的
   4. `Redis`-> 主流的缓存-读多写少的场景，同时容量受限于内存的大小
   5. `Sql Server`->仅支持 `windows`系统，也导致了它本身的局限性
   6. `MongoDB` -> 大数据文档存储，方便计算
   7. `HBASE` -> 超大`NoSql`存储机制，比`MongoDB`更大
   8. `ES` -> 搜索引擎-文档的检索服务
   9. `ClickHouse` -> 大数据列式存储，主要用作大数据场景进行数据报表分析使用
...
2. `Mysql`的主要作用是什么？
3. 数据库的三大范式是什么？
4. `MySql`的逻辑架构是什么样的（4层）？
5. `Mysql`的工作流程是什么样的，请详细描述？连接器、查询缓存、分析器、优化器、执行器
6. `MySql`中日志的录入格式是什么样的？
7. `MySql`的数据类型有哪些种类？
   1. 整数
      1. int（11），11有什么用？
