...
5. Redis如何做内存优化？（内存可能只有20G，但需要你放入原有30G，同时Redis本身的性能）
...
tomcat默认的最大线程数是200，如果单个接口的调用时间为 1s，那么这个接口的吞吐量在200/s，设置经验值：1核2G为200； 4核8G为800， 但是需要注意，线程数量越多，就意味着系统的稳定性越差。
