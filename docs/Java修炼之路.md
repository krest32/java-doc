...
2. `TypeScript`，简称TS，是`JS`的超集，能够完全的兼容`JS`，并在`JS`的基础上进行功能扩展，目前也是前端的当前的热门语言
3. `Python`通常会用来作为脚本类语言。因为类库繁多，它也是AI领域的大热门，缺点是性能比较差
4. `Java`的性能不错，同时支持网络编程和并发编程，主要用于Web后端开发，可以构建大型应用系统、游戏服务器。缺点是编译慢，运行时需要的内存比较高
5. `C`、`C++ ，流行了几十年的编程语言，可以直接调用系统资源，通常用来编写系统核心组件，但是上手难度比较高，
6. `Go`类似于`C` `C++，但是比更加简单，同时天然支持高并发，目前`Go`主要有两种方向
...
2. Finally 语句一定会被执行么？
3. 你们系统中异常是如何处理的？
   1. `try... catch.... finally`中finally使用return为什么是禁止的？
...
23. 为什么不直接使用`HashCode`作为table下标？
24. `HashMap`的扩容流程？
