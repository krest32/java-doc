...
+ 编译语言直接将程序转换为机器代码。结果，这些语言比解释器语言快得多。解释器花费很少的时间来分析源代码。但是，执行该过程的总时间要慢得多。与C或C ++的编译语言相比，这使Interpreted编程语言的执行非常慢。这就是为什么Python比C ++和Java慢的原因。Java既不是编译语言也不是解释语言。它位于中间。Java是与平台无关的语言，这使其在所有编程语言中都非常流行。字节码使其成为一种与平台无关的语言。这就是Java的优势。
...
7. `Integer`采用了使用了什么设计模式？
8. 接口和抽象类的区别？
9. 谈谈`API`和`SPI`之间的区别？
10. 成员变量和局部变量之间的区别是什么？
11. 在调用子类构造方法之前会先调用父类没有参数的构造方法，其目的是？
12. 静态变量、构造方法、成员变量三者的加载顺序？
13. 对象实例和对象引用之间的区别是什么？
14. 什么是内部类？为什么要有内部类？
15. 局部内部类的分类有哪些？
16. 构造方法是否可以被重写？可以被重载么？
17. `Java` 使用的值传递还是引用传递？二者有什么区别
18. Java中常用的包有哪些？
...
28. HashMap的那些操作会导致线程不安全？
29. `HashTable` 中 `key` `value` 为什么不能为`null`，而 `HashMap` 中却可以？
30. `HashTable`与`HashMap`之间的区别是什么？
31. `CopyOnWriteArrayList` 的具体实现原理 ？
32. `ConcurrentHashMap` 的底层实现原理 ？
33. 什么是迭代器 ？为什么要使用迭代器？
34. 使用迭代器的好处是什么？
...
14. 对称加密和非对称加密？
15. `SSL`和`TLS`之间的关系？
16. 简述`SSL`的工作方式？（SSL能让客户端和服务端非常有效的完成相互之间的认证工作）
17. 什么是`Http2`？
...
8. 什么是进程、线程、上下文切换（步骤、原因）？

9. 守护线程和工作线程之间的区别？

10. 什么是线程死锁？形成死锁的四个必要条件？
...
1. 市面上常见数据库的区别？
   ...
   2. `PostgreSQL`-> 最先进的
   3. `Oracle` -> 最完善的
   4. `Sql Server`->仅支持 `windows`系统，也导致了它本身的局限性
