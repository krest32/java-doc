...
7. 如何优化大分页查询？explain 字段的作用，还有常看哪些字段？
...
+ **分库分表**：
  ...
  + 如果一些数据按照时间维度统计，那么我们还可以按照年、月、日这样的维度来分,同时可以实现冷热数据分离
...
3. java 进程中CPU占用率100%的常见场景
   ...
   4. 大量线程、上下文切换；
4. 排查步骤
   1. 先找到占用CPU的进程；如果是Java进程，继续确定是哪个线程占用了CPU；
      1. 查看CPU的进程:top
      2. JPS 查看 java 进程
   2. 导出java进程的线程栈；PS：文件后缀名通常用tdump；
   3. jstack PID > PID.tdump 导出
   4. 根据线程ID从线程栈中找到对应线程栈；分析线程栈内容；
