...
24. `HashMap`在扩容上有哪些优化？
    1. 1.7以前都需要重新计算哈希，1.8以后，借助2备扩容机值，不需要重新计算哈希值，低位元素在原来位置，高位元素在原来位置+原来容量的大小
...
3. JVM的性能调优原则？
   ...
   + Gc优化是一个系统而复杂的工作，没有万能的调优策略，GC优化必须建立在对各种垃圾回收期的深入理解之上，才能达到事半功倍的效果
...
1. 热点问题，有一亿粉丝，如何推送状态？push or pull
   1. 如果是流量比较大的，最好使用pull的方式，因为这么多的粉丝不可能在同一时间取获取明星状态
   2. 如果是特别关注，那么需要采用推送的方式
      1. 第一种方案：可以客户端轮训去拉取这个消息
         + 会存在一个空轮训的问题
         + 轮训的频率是多久一次？如果一个小时的话，实时性会太差
      2. 维护一个推送的列表：多线程去推送这个消息到客户端
         1. 服务器的任务会很重，不太推荐
2. 一亿粉丝的如何存储？如何显示
   1. 数据量很大，所以肯定会做分库分表
   2. 查看一亿粉丝的列表，只返回一部分，比如某个库当中的一部分，不作全量查询
   3. 或者在缓存一部分数据，每隔一段时间更新
3. 某个热点突然爆火，那么爆火的热点要做一个数据的转存，到热点服务器

### B站弹幕设计（个人感觉）

**情况描述**

发送一条弹幕到直播房间，然后推送到其他的用户，实际上类似与即时聊天系统，
