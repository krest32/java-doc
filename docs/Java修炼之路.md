...
25. 如何停止一个正在运行的线程？`interrupted()` 和 `stop()`
26. 如何实现多线程之间的通讯和协作（两种方案）？
27. 线程死锁的理解？
28. 线程死锁的解决方案？
29. 什么是线程安全？
30. 如何保证线程安全？（3种方法）
...
34. 什么是锁的自旋？
35. 什么是锁的升级？
36. 谈谈你对 `AQS` 的理解吧？主要是它的一个实现原理？
37. Atomic 的运行原理 ？`CAS` 原理是什么？ 
38. `ABA`问题？
39. `CAS`带来的其他问题？
    1. 循环时间过长
    ...
    3. 只能保证一个共享变量的原子操作
40. 死锁、活锁、饥饿的区别？
41. 并发工具类Semaphore、`CountDownLatch`、`CyclicBarrier` 之间的区别？
42. 线程 B 怎么知道线程 A 修改了变量？ 线程之间的通信 （好多种方案）
    1.  volatile
    2. static
43. `volatile` 如何实现共享变量多线程的可见性？
44. `volatile` 如何实现`happens-before`规则？（内存屏障）
45. 线程池的 7 大参数？核心线程可以被销毁么？
46. `submit`和`excute`有什么区别？
47. 关闭线程池以及马上关闭线程池的方法是什么？它们之间有什么区别？
    1. `Showdown()`
    2. `Terminated()`
    3. `ShutdownNow()`
48. 线程池的饱和策略有哪四种？
49. 新任务到线程池以后，它的运行流程怎么样？
50. 并发工具类？
    1. `CountDownLatch`
    2. `CyclicBarrier`
    3. `Semaphore`
51. `int++`是线程安全的么？
52. `ThreadLocal`的使用场景？
53. 关于 `ThreadLocal` 可能引发的内存泄漏的问题，原因是什么？
54. `ForkJoin` 有用过么？思想是什么？
...
5. `Mysql` 锁：行锁、间隙锁（Gap Lock）、临界锁（Next-Key Lock）、表锁？
6. 为什么要避免间隙锁？
7. 什么情况下行锁升级表锁？
   1. 无索引 行锁 升级为 表
