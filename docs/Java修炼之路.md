...
+ 首先，技术的存在使为了解决某个特定的问题，所以谈论技术一定要结合实际使用场景
+ 其次，大多数情况下，一个技术方案很难做到十全十美，比如远程调用有Http和Rpc两种方式，http的使用范围一定会比Rpc更广泛，但是，如果系统对性能的要求比较高，那么必定会优先考虑Rpc。
+ 然后，技术会不停的更新，就以Mybatis和Mybatis-plus举例，如果没有前者，那么后者出现的时间，可能要晚几年，甚至也有可能不会被开发出来。在大多数业务开发的场景下，Mybatis-plus一定会更有优势；但是，如果企业需要对ORM框架进行一些定制化开发，我会优先考虑实用原生的Mybatis。
