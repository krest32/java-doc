...
1. JavaScript：简称JS，是前端必备的语言，完成和后端接口的数据交互，实现页面效果和数据的动态展示
2. TypeScript，简称TS，是JS的超集，能够完全的兼容JS，并在JS的基础上进行功能扩展，目前也是前端的当前的热门语言，不过最终编译之后运行的仍然是JS。
3. R数据分析、科学计算，这个语言的安全性很高，难度也比较大，会用于比较小的项目

**后端语言**

1. Python特点是简单、方便。因为类库繁多，它也是AI领域的热门，缺点是性能比较差，不过一般使用Python的人通常使看中了它的便利性。
2. Java的性能不错，生态丰富，支持网络编程和并发编程，主要用于Web后端开发，构建大型应用系统。缺点是编译慢，运行时需要的内存比较高
3. C、C++，流行了几十年的编程语言，可以直接调用系统资源，通常用来编写系统核心组件，但是上手难度比较高，最关键的是它的性能高，且没有自动的垃圾回收机制，这也就避免了因为自动垃圾回收造成的程序卡顿。所以高级语言成就在垃圾自动回收，瓶颈却也在垃圾回收
4. Go类似于C C++，但是比更加简单，同时天然支持高并发，目前Go主要有两种方向
...
+ 首先：技术的存在目的使为了解决某个特定的问题而存在的，所以我的想法使不能脱离场景谈技术
+ 其次：大多数情况下，一个技术方案很难做到十全十美，比如远程调用有Http和Rpc，在使用范围上面，http一定比Rpc广，但是如果对性能有要求，那么必定优先考虑Rpc。
+ 然后：技术会不停的更新，就以Mybatis与Mybatis-plus举例，如果没有前者，那么后者出现可能要晚几年，甚至可能不会出现。
+ 最后：同一个功能，但是不同的场景下，对技术的选型会提出不同的要求，就比如Mybatis与Hibernate，如果业务的Sql相对来说比较简单且要求快速交付，那么我应该会选择hibernate；反之业务查询数据库的逻辑比较复杂，且对Sql的性能有一定要求，这个时候我会选择Mybatis。
...
4. 你常用的 Linux 指令有哪些？
   1. 文件管理：mkdir、mv、rm等 
   2. 查看或者编辑文件：cat、tail、vim等
   3. 压缩：tar、unzip等
   4. 系统操作：top、systemlctl、ps等
   5. 用户管理相关指令
5. 程序一般安装和使用流程
   1. 新建用户
   2. 添加文件夹，设置文件归属
