...
4. `Java`程序执行的顺序是什么样的？
5. `JDK 8` 的主要特性有哪些呀？（JVM 元空间、Stream流、CompletableFuture、lambda、Local Time包）
...
43. 序列化有什么用？`serivalVersionUUID` 作用？ `transient` 的作用？
44. 被`transient `修饰变量的初始值会是什么？
45. `Java` 序列化的缺点有哪些？
46. 分布式系统序列化的发展历史？
47. 对于反射的理解，应用场景有哪些？
48. 反射的优缺点有哪些？
...
51. 为什么要使用日志框架？
52. 日志的输出级别有哪些？
53. 最近关于 `log4j2` 的`bug`有了解过么？漏洞补救措施，漏洞解决措施？
54. 谈谈你对`Java` 异常的理解？
55. `Java` 异常的分类有哪些？
56. `JVM` 时如何处理异常的？
57. 什么时受检时异常？
...
60. 什么是泛型？
61. 为什么要使用泛型？
62. 常用通配符代表的含义 (`T` `?` `K V` `E`)?
63. 什么是类型擦除？
64. 如何在`List<Integer>`中插入字符串或者浮点数？   
