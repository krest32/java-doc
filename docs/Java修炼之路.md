...
互联网野蛮生长的时代已经过去，日益复杂的国际形式也给经济发展带来了新的挑战。中国当下的核心目标从低端制造业中脱离出来，完成产业升级。我个人从事后端开发工作，未来几年信息产业方面如何发展也会与我自身息息相关的，接下来我会看好以下行业：

1. 人工智能：
   1. ChatGPT这类的生成式AI，目前仍是大热门
   2. 行业AI持续发力，比如
      1. 各大汽车公司以及第三方的自动驾驶技术
      2. 小米的物联网和全屋智能
2. 大数据挖掘分析：金融系统、医疗数据、推荐算法等
3. 政企数字化建设，这类需要一直都会有，但是需求不会向前几年那么多
   1. IT为各个行业进行赋能，比如：智慧校园、智慧水电等
   2. 部分技术型企业，自研产品或系统

总结：大数据和人工智能会是未来国家战略性的产业。跟着国家的方向走，总归是不会出错的，属于目前的蓝海行业，但是入行门槛比较高，并且对于一般的传统公司小型企业来说，实用性不高，所以结果就是，就业岗位比较稀缺。

再说前后端岗位，互联网发展这么多年，it技术相对来说已经很成熟了，而且你在工作中遇到的问题，其中99%都会有现成解决方案。it行业目前来说，不仅仅卷技术，甚至已经开始卷学历和年龄了，一些公司，在招聘的时候，直接就会有学历限制和年龄限制。未来几年，经济如果持续下行，这个行业的竞争压力仍然会持续增加，其实我觉得挺卷的，但IT还不至于走下铺路，毕竟计算机时代，需求或多或少，未来几年一直都会有。



## 前后端工作本质

这些年工作下来，我对于编程的理解就比较简单，概述下来就是：我们不生产数据，只是数据的搬运工。

+ 前端工作的核心就是将数据展示给用户。
+ 后端工作的本质就是在浏览器与服务器之间，完成数据的存储和搬运工作

这个过程类似于我们在电脑中打开一个word文档，然后进行一系列的新增、修改、删除等操作。只不过通过编程我们可以实现在本地对远端数据的进行类似操作。

因为要展示数据，所以需要浏览器，比如谷歌、火狐等

因为实现交互数据的逻辑，所有有了前后端编程语言，比如JavaScript、Java、Golang、Python、C、C++等

因为要存储数据，所以有了数据库，比如 Mysql、Orcal等

IT的工作并没有你想的那么高大上，它工作的本质其实就是记录数据，只不过经过时间的发展，记录的方式已经发生了天翻地覆的变化
...
18. 对于`final`关键字的理解？
19. 对于`static`关键字的理解？
20. `this`和`super`之间的区别是什么？
21. ==`和`equal`之间的区别？
22. 对象比较时，为什么要重写`Equal()方法后，一定要重写`hashcode()方法？
23. `String`为什么会比`StringBuilder`慢？
24. `String`为什么要设计为不可变类？
    1. 线程安全
