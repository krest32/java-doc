...
3. `Python` 日常的生活问题、脚本类语言非常合适，因为类库多，也被用于人工AI领域>机器学习>深度学习等领域，因为不需要太高的性能，而且python本身包含很多的数学模型计算公司，所以Python很流行，尤其在职能领域
...
谈谈你对于 Java 的理解？为什么是`Java`而不是其他？有什么优势？

Java已经被发明很多年了，经过这么长时间的验证，Java语言以及它庞大的生态体系决定的它的领域所在就是构建大型业务系统，而且市面上Java的份额最大，相较于其他语言，学习Java能够更快速且容易找到工作

Java语言面向对象设计，整体相对比较简单，且功能强大，支持网络编程，基于Jvm可以让使用者不用过多关心内存泄漏问题，而且可以跨平台使用，这些内在因素决定了我想要去掌握这么语言。

#### 基础

1. `Jdk 1.5`之后的三大版本？
2. `JVM`、`JDK`、`JRE `都有什么区别？
3. 为什么说Java语言编译与解释共存？
4. `Java`程序执行的顺序是什么样的？
5. `JDK 8` 的主要特性有哪些呀？（JVM 元空间、Stream流、lambda、CompletableFuture、新的时间包：LocalDateTime）
6. 为什么要固守使用 `JDK 8`？对比 `JDK 11`？对比 `JDK 17` ?
7. 可以说一下对于JDk 19 的了解么？
8. `Java` 的八种基本数据类型有哪些？
9. 最有效的计算2乘以8？
10. `switch` 能否用在`String`上？那么`Long`呢？为什么不支持`Long`？
11. `null`是什么？
12. 什么是不可变类？常见的不可变类有哪些？
13. 什么是数组？数组有哪些特点？
14. `String` 为什么会比 `StringBuilder` 慢？
15. 对于 `final` 关键字的理解？
16. 对于 `static` 关键字的理解？
...
24. `HashMap`在扩容上有哪些优化？
    + 1.7以前都需要重新计算哈希，
    + 1.8以后，借助2备扩容机值，不需要重新计算哈希值，低位元素在原来位置，高位元素在原来位置+原来容量的大小

25. `HashMap`中的链表转化红黑树需要哪两个条件？
26. 为什么`String`和`Integer`适合作为`Map`的`Key`?
27. 可以使用`Object`对象作为`key`么？
28. 关于 `TreeMap` 的理解说一下？新元素插入需要提起准备什么？
29. HashMap的那些操作会导致线程不安全？
30. `HashTable` 中 `key` `value` 为什么不能为`null`，而 `HashMap` 中却可以？
31. `HashTable`与`HashMap`之间的区别是什么？
32. `CopyOnWriteArrayList` 的具体实现原理 ？
33. `ConcurrentHashMap` 的底层实现原理 ？
34. 什么是迭代器 ？为什么要使用迭代器？
35. 使用迭代器的好处是什么？
...
37. 什么是锁的升级？
38. 重量级锁的实现原理？
39. 谈谈你对 `AQS` 的理解吧？主要是它的一个实现原理？
40. Atomic 的运行原理 ？`CAS` 原理是什么？ 
41. `ABA`问题？
42. `CAS`带来的其他问题？
    1. 循环时间过长
    ...
    3. 只能保证一个共享变量的原子操作
43. 死锁、活锁、饥饿的区别？
44. 并发工具类Semaphore、`CountDownLatch`、`CyclicBarrier` 之间的区别？
45. 线程 B 怎么知道线程 A 修改了变量？ 线程之间的通信 （好多种方案）
    1.  volatile
    2. static
46. `volatile` 如何实现共享变量多线程的可见性？
47. `volatile` 如何实现`happens-before`规则？（内存屏障）
48. 线程池的 7 大参数？核心线程可以被销毁么？
49. `submit`和`excute`有什么区别？
    1. submit 执行 callable，submit 可以有返回值、异常
    2. excute 执行 runnable、excute 没有返回值异常

50. 关闭线程池以及马上关闭线程池的方法是什么？它们之间有什么区别？
    1. `Showdown()`
    2. `Terminated()`
    3. `ShutdownNow()`
51. 线程池的饱和策略有哪四种？
52. 新任务到线程池以后，它的运行流程怎么样？
53. 并发工具类？
    1. `CountDownLatch`
    2. `CyclicBarrier`
    3. `Semaphore`
54. `int++`是线程安全的么？
55. `ThreadLocal`的使用场景？
56. 关于 `ThreadLocal` 可能引发的内存泄漏的问题，原因是什么？
57. `ForkJoin` 有用过么？思想是什么？
...
1. 市面上常见数据库的区别？
   ...
   2. `PostgreSQL`-> 最先进的
   3. `Oracle` -> 最完善的
   4. `Redis`-> 主流的缓存-读多写少的场景
   5. `Sql Server`->仅支持 `windows`
   6. `MongoDB` -> 大数据文档存储
   7. `ES` -> 搜索引擎-文档的检索服务
   8. `ClickHouse` -> 大数据列式存储
...
4. `Mysql`的工作流程是什么样的，请详细描述？连接器、查询缓存、分析器、优化器、执行器
...
6. `MySql`的数据类型有哪些种类？
   ...
   5. 时间日期类
      ...
      2. `datetime`
7. 设置int的长度有什么用？
8. 如何选择字符类型？（`char`、`varc har`、`text`）

**存储引擎与索引**

1. `MyIASM`对比`InnoDB`？
2. 为什么`MyIASM`查询上会比`Innodb`稍微快些？
3. `InnoDB`的四大特性有哪些，各自起到了什么的作用？
4. 如何选择存储引擎？
5. 什么是索引？它优缺点有哪些？
6. 索引的分类？
...
18. 为什么数据量很大的时候Mysql的性能会下降（为什么推荐的数据量在2000万左右）？
19. 谈谈对于叶分裂的理解？
...
1. ACID是靠什么保证的？
2. 什么是脏读、幻读、不可重复读？
3. 谈谈你对`MySql`事务的隔离级别的理解？
4. 当前读和快照读？如何实现当前读（共享锁或排它锁）？
5. 隔离级别和锁之间的关系？
6. `Mysql` 锁：行锁、表锁、页锁、元数据锁？
7. InnoDB行锁的分类有哪些？
8. InnoDB的行锁模式？
9. 为什么要避免间隙锁？
10. 什么情况下行锁升级表锁？
...
3. `RDB`持久化的工作流程？
4. `AOF`持久化机制的特点？
5. `AOF`什么时候写入磁盘？（always\EverySec\No)
6. 什么是`AOF`的重写机制？
7. 如何使用合适的持久化方式？
8. `RDB`与`AOF`是如何混合使用的？
9. 如何选择合适的持久化策略？
...
2. `Redis`的内存淘汰策略有哪些？
3. 使用Redis如何作内存优化？（内存可能只有20G，但需要你放入原有的）
4. Redis回内存收使用的是什么算法？
...
6. `#{}`  与 `?{}` 之间有什么区别？
7. `Mybatis` 的一级缓存与二级缓存有什么理解 ？作用域有什么区别？（`session`、`nameSpace<cache>标签`）
8. `Mybatis`插件实现原理？
...
3. 核心注解有哪些？`@SpringBootApplication` 、`@EnableAutoConfiguration`、`@ComponentScan`、`@ConditionOnClass`
4. SpringBoot自动配置原理？
...
+ 第二阶段：切换到G1，这种情况变少了，但是会有内存使用使用率比较的情况
