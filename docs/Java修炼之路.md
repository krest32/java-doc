...
6. 为什么要使用缓存池？缓存池在什么位置？采用了使用了什么设计模式？
7. 对象实例和对象引用之间的区别是什么？
8. 成员变量和局部变量之间的区别是什么？
9. 在调用子类构造方法之前会先调用父类没有参数的构造方法，其目的是？
10. 什么是内部类？为什么要有内部类？
11. 局部内部类的分类有哪些？
12. 构造方法是否可以被重写？可以被重载么？
13. 接口和抽象类的区别？
14. `Java` 使用的值传递还是引用传递？二者有什么区别
...
2. 为什么要使用日志框架？
3. 日志的输出级别有哪些？
4. 最近关于 `log4j2` 的`bug`有了解过么？漏洞补救措施，漏洞解决措施？
...
41. `volatile` 如何实现happens-before规则？
42. 线程池的 7 大参数？核心线程可以被销毁么？
43. `submit`和`excute`有什么区别？
44. 关闭线程池以及马上关闭线程池的方法是什么？它们之间有什么区别？
    1. `Showdown()`
    2. `Terminated()`
    3. `ShutdownNow()`
45. 线程池的饱和策略有哪四种？
46. 新任务到线程池以后，它的运行流程怎么样？
47. 并发工具类？
    1. `CountDownLatch`
    2. `CyclicBarrier`
    3. `Semaphore`
48. `int++`是线程安全的么？
49. `ThreadLocal`的使用场景？
50. 关于 `ThreadLocal` 可能引发的内存泄漏的问题，原因是什么？
51. `ForkJoin` 有用过么？思想是什么？
