...
8. 简述一下分代算法？
9. `CMS` 工作流程是什么样子的？
10. 什么是三色标记法？（代表对象被清理的过程）
11. `STW` 一般发生在程序的什么时候 ？
12. `G1` 的算法原理是什么？相比分代算法的优势？对比`CMS`又有什么样的特点？
13. 了解过 `ZGC` 么？（专门针对超大数据量的垃圾回收器）
14. 什么是`GC`的吞吐量？（一次尽可能多的回收垃圾）
15. 什么是`GC`的低延迟？（每次回收垃圾的时间都很短）
16. 在配置垃圾回收器的时候，场景该如何配置？
