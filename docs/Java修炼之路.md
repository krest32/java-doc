...
用一句话简单来说明：我们不生产数据，我们只是数据的搬运工。

+ 前端程序员的工作是将数据展示给用户。
+ 后端程序员的工作是完成数据的转换、传输和存储工作

一个程序工作的过程类似于我们在电脑中打开一个文档，然后进行一系列的新增、修改、删除等操作。而程序员通过编写代码，可以使本地电脑访问远端电脑存储的文档，然后进行类似操作。

+ 因为要展示数据，所以需要浏览器，比如谷歌浏览器和火狐浏览器等。
+ 因为完成不同电脑之间的数据交互，所以我们需要使用一些的编程语言，比如JS、Java、Golang、Python、C、C++等
+ 因为要存储数据，所以会用到数据库，比如Mysql、Orcal等

程序员的工作其实并没有很多人想得那么复杂，它的本质其实是帮助人们完成对数据进行一系列操作而已。
...
1. Python特点是简单、方便。因为类库繁多，所以很多场景都有它的身影，目前它也是AI领域的热门，缺点是性能比较差。
2. Java的性能不错，生态丰富，支持网络编程和并发编程，主要用于快速构建大型Web服务器系统；缺点是编译慢，运行时需要的内存比较高
3. C、C++，流行了很多年的编程语言，通常用来编写系统核心组件，但是上手难度比较高，最关键的是它的性能高，且没有自动的垃圾回收，这也就避免了因为自动垃圾回收造成的程序卡顿。
4. Go的特点是简单、高效，天然支持高并发，目前Go主要有两种方向
   1. 第一个是云原生
   2. 第二个是核心高并发业务
...
我最近在看一些开源的框架，其中比较重要的就是Spring，其实早在2年前，我就大体上看了它的源码，这一次重新再看，是因为工作有涉及到框架的开发。

我以前是做业务开发的，也写过两个开源产品，但是对于框架开发，我也是第一次，然后今天想聊一下框架开发的感受：

+ 第一个是代码质量，框架需要不停的维护、更新迭代的，所以代码质量的要求必定会比业务代码更高一些
+ 第二个是功能的可扩展性，通常两说，扩展方式有两种，一种是链式扩展，比如Filter；另一种是星式扩展，比如Listener
+ 第三个是框架的性能，通常来说，框架的灵魂技巧是反射，但是反射的性能一直被诟病，所以慎用反射。
+ 第四个是资源的预处理，比如配置文件的加载，SPI资源类的注册解析等等

就以开源来说，Spring无论在代码质量还是可扩展方面都是非常好的，每一次阅读，都能从中学到一些新的东西。
