...
2. 什么是面向对象开发，什么是面向过程，二者对比有什么区别？
3. 面向对象开发的基本原则是什么？（六大原则）
4. 为什么要有基本数据的包装类？
5. `Integer` 缓存池有了解过么 ？
6. `new Integer` 和 `Integer.valueOf()` 之间的区别是什么？一般在项目中哪一个？
7. 为什么要使用缓存池？缓存池在什么位置？
8. `Integer`采用了使用了什么设计模式？
9. 接口和抽象类的区别？
10. 谈谈`API`和`SPI`之间的区别？
11. 成员变量和局部变量之间的区别是什么？
12. 在调用子类构造方法之前会先调用父类没有参数的构造方法，其目的是？
13. 静态变量、构造方法、成员变量三者的加载顺序？
14. 对象实例和对象引用之间的区别是什么？
15. 什么是内部类？为什么要有内部类？
16. 局部内部类的分类有哪些？
17. 构造方法是否可以被重写？可以被重载么？
18. `Java` 使用的值传递还是引用传递？二者有什么区别
19. Java中常用的包有哪些？
...
7. 谈谈你对Linux的五种IO模型分别是什么？
   ...
   1. Linux的多路复用IO有哪些种类？epoll 为什么性能高？
      ...
      1. select 和 poll 都是基于轮询的方式
         ...
         epoll 基于事件驱动的方式，性能比前两者更高一些
   1. 信号驱动IO与多路复用IO之间的区别
   
8. Reactor IO 的重要组件，以及模型三种分类？
...
1. 尽可能缩短键值对的长度，减少内存在占用，也可以存储2进制的形式
2. 使用延迟删除特性，避免频繁删除
3. 设置键值对的过期时间
4. 禁止耗时长的查询命令
5. 使用Slowlog优化耗时命令
...
+ 原因可能存在两个
  ...
  + CPU频繁上下文切换，系统中通常会有文件IO、网络IO、锁等
