...
4. `Java`程序执行的顺序是什么样的？
5. `JDK 8` 的主要特性有哪些呀？（JVM 元空间、Stream流、CompletableFuture、lambda、LocalTime LocalDate包）
...
15. 对于 `final` 关键字的理解？
16. 对于 `static` 关键字的理解？
17. `try... catch.... finally` 中 ` finally`  使用 `return` 为什么是禁止的 ？
18. 谈谈你对于枚举的理解？
19. `this` 和`super`之间的区别是什么？
20. `==` 和 `equal` 之间的区别？
21. 对象比较时，为什么要重写 `Equal()`  方法后，一定要重写 `hashcode()` 方法？
22. 什么是面向对象开发，什么是面向过程，二者对比有什么区别？
23. 面向对象开发的基本原则是什么？（六大原则）
24. 为什么要有基本数据的包装类？
25. `Integer` 缓存池有了解过么 ？
26. `new Integer` 和 `Integer.valueOf()` 之间的区别是什么？
27. 为什么要使用缓存池？缓存池在什么位置？采用了使用了什么设计模式？
28. 对象实例和对象引用之间的区别是什么？
29. 成员变量和局部变量之间的区别是什么？
30. 在调用子类构造方法之前会先调用父类没有参数的构造方法，其目的是？
31. 什么是内部类？为什么要有内部类？
32. 局部内部类的分类有哪些？
33. 构造方法是否可以被重写？可以被重载么？
34. 接口和抽象类的区别？
35. `Java` 使用的值传递还是引用传递？二者有什么区别
36. 什么是 `I/O` ？
37. `I/O`流主 要有哪些分类？
38. 谈谈你对于IO有哪些认识？
39. `BIO`、 `NIO`、`AIO` 是什么？有什么区别？
40. `BIO`、`NIO`的使用场景？
41. 字符流与字节流有什么区别，用到了什么设计模式？
    1. 字符流拥有缓存区
    2. 字符流方便我们操作数据 
42. 多路复用 `I/O` 模型的概念？它有什么局限性？
43. 谈谈你对Linux的I/O有哪些理解？
44. 序列化有什么用？`serivalVersionUUID` 作用？ `transient` 的作用？
45. 被`transient `修饰变量的初始值会是什么？
46. `Java` 序列化的缺点有哪些？
47. 分布式系统序列化的发展历史？
    1. SOP协议 + WebSocket
    2. Restful + Json
...
52. 为什么要使用日志框架？
53. 日志的输出级别有哪些？
54. 最近关于 `log4j2` 的`bug`有了解过么？漏洞补救措施，漏洞解决措施？
55. 谈谈你对`Java` 异常的理解？
56. `Java` 异常的分类有哪些？
57. `JVM` 时如何处理异常的？
58. 什么时受检时异常？
...
61. 什么是泛型？
62. 为什么要使用泛型？
63. 常用通配符代表的含义 (`T` `?` `K V` `E`)?
64. 什么是类型擦除？
65. 如何在`List<Integer>`中插入字符串或者浮点数？   
...
17. 谈谈你对`WebSocket`的理解有哪些？
18. 什么是 `Socket` ？
19. 什么是 `Servlet` ？它是线程安全的么？它的生命周期？
...
10. 什么是线程死锁？
11. 如何创建线程呢？
12. 同一个线程可以两次调用`start()`方法么？
13. `runnable` 接口与`callable`接口之间的区别？
14. `run()` 和`start()`之间的区别？
15. 什么是`Callable`、`Future`、`FutureTask`？ 
16. `Future`、`FutureTask` 与 `Completefutere` 的区别？
17. 线程的五种状态？
18. 线程的调度算法有哪些？
19. 常用的线程操作方法 `sleep` 、`wait`、`notify`、`notifyAll`，简单聊一下他们的使用？
20. 为什么`wait`、`notify`、`notifyAll`是 `Object`方法？
21. 关于 `sleep() `方法 与 `yeild()` 方法有什么区别？
22. 如何停止一个正在运行的线程？`interrupted()` 和 `stop()`
23. 如何实现多线程之间的通讯和协作（两种方案）？
24. 线程死锁的理解？
25. 线程死锁的解决方案？
26. 什么是线程安全？
27. 如何保证线程安全？（3种方法）
28. 线程过多会导致什么问题？
29. synchronized 与 lock 区别？
30. 什么是锁消除？
31. 什么是锁的自旋？
32. 什么是锁的升级？
33. 谈谈你对 `AQS` 的理解吧？主要是它的一个实现原理？
34. Atomic 的运行原理 ？`CAS` 原理是什么？ 
35. `ABA`问题？
36. `CAS`带来的其他问题？
    1. 循环时间过长
...
37. 死锁、活锁、饥饿的区别？
38. 并发工具类Semaphore、`CountDownLatch`、`CyclicBarrier` 之间的区别？
39. 线程 B 怎么知道线程 A 修改了变量？ 线程之间的通信 （好多种方案）
    1.  volatile
    2. static
40. `volatile` 如何实现共享变量多线程的可见性？
41. 线程池的 7 大参数？核心线程可以被销毁么？
42. `submit`和`excute`有什么区别？
43. 关闭线程池以及马上关闭线程池的方法是什么？它们之间有什么区别？
    1. `Showdown()`
    2. `Terminated()`
    3. `ShutdownNow()`
44. 线程池的饱和策略有哪四种？
45. 新任务到线程池以后，它的运行流程怎么样？
46. 并发工具类？
    1. `CountDownLatch`
    2. `CyclicBarrier`
    3. `Semaphore`
47. `int++`是线程安全的么？
48. `ThreadLocal`的使用场景？
49. 关于 `ThreadLocal` 可能引发的内存泄漏的问题，原因是什么？
50. `ForkJoin` 有用过么？思想是什么？
...
22. 四种引用类型的区别？
23. 谈谈你对`GC`的理解？
24. 促发`Full GC`的条件有哪些？
25. 垃圾回收算法有哪些？
26. 简述一下分代算法？
27. `CMS` 工作流程是什么样子的？
28. 什么是三色标记法？（代表对象被清理的过程）
29. `STW` 一般发生在程序的什么时候 ？
30. `G1` 的算法原理是什么？相比分代算法的优势？对比`CMS`又有什么样的特点？
31. 了解过 `ZGC` 么？
32. 什么是`GC`的吞吐量？
33. 什么是`GC`的低延迟？
34. 在配置垃圾回收器的时候，场景该如何配置？
35. `Java`为什么解释与编译并行？
36. 什么时候，会判断当前代码是热点方法？
37. 谈谈你对类加载机制的了解？
38. 有哪些类加载器？
39. 为什么是双亲委派机制？类加载的流程？
40. 为什么要自定义类加载器？
41. 如何自定义类加载？
42. 什么时候虚拟机会退出？
...
10. 索引的分类有哪四种？
11. 索引一定能够提高查找性能么？
12. `Innodb` 索引的数据结构有哪些？
13. 为什么是B+树，它对比B树、Hash索引的优缺点？
14. B+树与红黑树之间的区别是什么？
15. 为什么数据量很大的时候Mysql的性能会下降？
16. 什么是叶分裂，什么情况下会导致叶分裂？
17. 什么情况下可以使用`Hash`索引？
18. 索引设计的原则，以及如何设计索引？
19. 如何避免回表？
    + 使用索引覆盖，把所有的列都放到索引中，这样存储引擎不需要回表获取对应行就可以返回结果
20. 什么是最左匹配原则？
21. 什么情况下联合索引可以乱序，不需要最左匹配？
22. 什么是事务，事务的ACID代表什么？
23. 什么是脏读、幻读、不可重复读？
24. `MySql`事务的隔离级别有哪些？
25. 当前读和快照读？如何实现当前读（共享锁或排它锁）？
26. `Mysql` 锁：行锁、间隙锁（Gap Lock）、临界锁（Next-Key Lock）、表锁？
27. 为什么要避免间隙锁？
28. 什么情况下行锁升级表锁？
    1. 无索引 行锁 升级为 表
...
35. `Mysql` 主从复制原理
36. 分库分表实现方式
37. 分库分布之后的跨库join查询如何实现？
38. 分库分表以后跨库的分页查询如何实现？
...
8. 什么是`AOF`的重写机制？
9. 如何使用合适的持久化方式？
10. `RDB`与`AOF`是如何混合使用的？
11. 如何选择合适的持久化策略？
12. 关于`Redis`的线程模型是什么样的？
13. `Redis`为什么不使用多线程？
14. 关于`Redis`的事务了解多少？
15. `Redis`的过期键的删除策略是什么样的？
...
21. `Redis` 如何实现分布式锁？
22. `RedLock`（红锁）的实现原理是什么？
23. `Redis` 的主从架构？
24. `Redis` 的哨兵模式？
25. `Redis` 集群？
26. `Redis` 的客户端工具：`Redisson` 、`Jedis`、 `Lettuce`
    1. 分布式协调客户端

27. `Rediss` 的分布式锁与`Setnx`直接的区别？
    1. `Setnx` 需要手动设定一个超时时间，避免业务死掉造成死锁
    2. `Redisson` 提供 看门狗，自动增加超时时间

28. 如何保证数据库与缓存的双写一致性?
29. `Redis` 一个Key 能够存储的最大容量？
30. `Redis` 可以做异步队列么？
31. `Redis` 如何实现延时队列的功能 ？
32. `Redis` 中的跳表是什么样子的？
...
4. `#{}`  与 `?{}` 之间有什么区别？
5. `Mybatis` 的一级缓存 与 二级缓存 有什么理解 ？作用域有什么区别？（`session`、`nameSpace`）
6. `Mybatis`插件了解多少？
