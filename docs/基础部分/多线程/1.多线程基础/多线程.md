...
![aHR0cHM6Ly91c2VyLWdvbGQtY2RuLnhpdHUuaW8vMjAxOS8xMS8yNS8xNmVhMDQ3NjEyOTVlNzY2](img/aHR0cHM6Ly91c2VyLWdvbGQtY2RuLnhpdHUuaW8vMjAxOS8xMS8yNS8xNmVhMDQ3NjEyOTVlNzY2.png)
...
- CountDownLatch 所有线程一起完成。
  ...
  - 问题：但是Await方法可能会引起死锁，在高并发的场景下，会出现多个请求占用了全部的线程，但是每个请求在需要Await其他线程返回结果，被等待的线程拿不到线程资源，从而造成多个请求无法进入阻塞，从而造成死锁
  - 解决：使用自定义线程池扩大线程数量，并建立线程池拒绝机制来解决死锁
...
- CountDownLatch方法比较少，操作比较简单，而CyclicBarrier提供的方法更多，比如能够通过
  ...
  - getNumberWaiting()，isBroken()这些方法获取当前多个线程的状态，
  - 并且CyclicBarrier的构造方法可以传入barrierAction，指定当所有线程都到达时执行的业务功能；

- 二者的功能相似，但是**CountDownLatch是不能复用的，而CyclicLatch是可以复用的。**

  - 场景：需要计算N组N个人数值时，很明显CyclicLatch会更加合适，它可以处理更加复杂的业务场景，同时也更加灵活
