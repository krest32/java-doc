...
- CyclicBarrier 称为同步屏障，也是一个同步辅助类，允许一组线程彼此互相等待，直到到达某个公共的屏障点，此屏障在等待线程被释放之后可重用，所以称它为可循环使用的屏障。
...
- 总结：
  - CountDownLatch：自己是主管，让小弟去干活，等小弟干完，主管再向下执行
  - CyclicBarrier：让一批线程自己等待，等到所有的线程都准备好了，自己才继续（或者说一起执行）
