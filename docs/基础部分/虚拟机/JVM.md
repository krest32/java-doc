...
+ 在开启压缩指针情况下，会有12个字节，但是JVM会按照8个字节的倍数进行填充，所以会变成16个字节
...
+ 老年代的收集器包括Serial Old、Parallel Old、CMS，回收整个Java堆的G1收集器。
