...
1. 使用方面：
   - CentOS适用于服务器，在国内的linux 运维环境中， centos 是占大头的。一方面是因为centos出生名门，作为redhat社区版本，其稳定性毋庸置疑。
   - Ubuntu则适用于个人桌面，在国际上，采用 centos 和采用 ubuntu 作为服务端是其实是五五开的。
2. 用户方面
   - Ubuntu适合初学者,有一个很大的社区平台,为我们提供丰富的文档和经验，服务器可以图形化界面都适合大部分人的习惯。
   - CentOS更适合公司生产环境的使用，更新频率不高，只有稳定的版本才会发布。网上的项目教程有很多是基于Centos。
3. 稳定性
   - CentOS 更加稳定，服务器部署多，文档丰富，对运维友好
   - Ubuntu 功能更新，图形桌面支持更好，对开发和个人用更友好，追求最新功能。内核版本版本较新，新的功能能及时推出同步
   - 当然随着docker崛起，这种差异也在逐渐较少。由于ubuntu内核版本更新，而docker容器对linux内核新功能要求比较高，所以使用ubuntu作为docker容器的宿主机更加友好一点。
4. 策略方面
   - Ubuntu上手容易，最主要的是文献较多，你使用过程中遇到的任何问题随便搜，都会找到好多答案，所以，从易于使用方面来讲，Ubuntu优于centos。你是当作办公用，写写文档，上上网，听听歌，那还是用Ubuntu啊!Ubuntu虽不是Windows那样的桌面操作系统，但是，它还是为使用桌面而做足了功课，尤其是办公使用，只要你要求不高，它也足够用了。
   - 作为服务器使用还是用centos，不是说ubuntu不能用，如果用win7当作服务器上线总是感觉不放心啊，什么时候就死机了呢。服务器操作系统要求的稳定、强壮的运行！
5. 总结：**CentOS适合于服务器**，**Ubuntu更合适于个人使用**
...
![在这里插入图片描述](img/20200229173922281.png)![在这里插入图片描述](https://img-blog.csdnimg.cn/20200229173922281.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1RoaW5rV29u,size_16,color_FFFFFF,t_70)
...
![image-20230908161238690](img/image-20230908161238690.png)
...
# 查看創建了哪些用戶
cat /etc/passwd
# 此时home下还存在相应的文件夹但是用户被删除
userdel 需要删除的用户
# 此时home下相应的文件夹也被删除
userdel -r 需要删除的用户
# 修改用戶名
usermod -l new_username(新用户名) old_username(要修改的用户名)
...
~~~bash
# 给指定用户或组授权
setfacl   -m  u：用户或用户组：权限     文件
# 给zhangsan授予可以对文件读写执行的权利
setfacl -m u:zhangsan:rwx  /test/t1    
# 给news组授予对文件t1读写权限
setfacl -m g:S1:rw /test/t1              
...
# 将文件的归属给到某个用户
chown -R sam:sam /root/test/test1
~~~
...
Linux 支持 5 种文件类型，如下图所示：![文件类型](img/aHR0cDovL3N0YXRpYzIuaW9jb2Rlci5jbi9iNjg2NTQxN2RkMTI0NWU0ZjNjNGJhODc3Y2U5YzVhYQ)

### Linux 的目录结构是怎样的？

> 这个问题，一般不会问。更多是实际使用时，需要知道。

Linux 文件系统的结构层次鲜明，就像一棵倒立的树，最顶层是其根目录： ![Linux的目录结构](img/aHR0cDovL3N0YXRpYzIuaW9jb2Rlci5jbi81YzJmNTRmMjhkNGVhYmJlMTM4ZmQ4Y2JiNzYzYTFmMQ)
...
4. 每一个文件都有对应的inode，里面包含了与该文件有关的一些信息。
...
如同 `for` 循环，`while` 循环只要条件成立就重复它的命令块。 不同于 `for`循环，`while` 循环会不断迭代，直到它的条件不为真。
...
![img](img/aHR0cHM6Ly93d3cucnVub29iLmNvbS93cC1jb250ZW50L3VwbG9hZHMvMjAxNC8wNy92aW0tdmktd29ya21vZGVsLnBuZw)
...
系统类的常用会有 ifconfig、top、kill、grep、df、 ls 、ll

下载资源类： yum、wegt

解压文件：tar 、unzip

操作文件或者文件夹：rm、 mv、 mkdir、 cd 、chmod 

操作文件内容或查看：cat、 tail 、vim
