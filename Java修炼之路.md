...
31. 字符流与字节流有什么区别，用到了什么设计模式？
    1. 字符流拥有缓存区
    2. 字符流方便我们操作数据 
32. 多路复用 `I/O` 模型的概念？它有什么局限性？
33. 对于反射的理解，应用场景有哪些？
34. 反射的优缺点有哪些？
35. 获取反射的几种方法？
36. 如何提高反射的效率 ？
37. 序列化有什么用？`serivalVersionUUID` 作用？ `transient` 的作用？
38. 被`transient `修饰变量的初始值会是什么？
39. 分布式系统如何进行序列化操作？
40. `Java` 序列化的缺点有哪些？
41. 为什么要使用日志框架？
42. 日志的输出级别有哪些？
43. 最近关于 `log4j2` 的`bug`有了解过么？漏洞补救措施，漏洞解决措施？
44. 什么是`Java` 异常？
45. `Java` 异常的分类有哪些？
46. `JVM` 时如何处理异常的？
47. 什么时受检时异常？
...
7. `TCP` 报文有了解过么？
8. 谈谈你对于`Http` 请求方式的理解（4中请求方式）？`Get/Post`
9. 常见的`Http`请求状态码？比如 400、401、403、404、501、502、503
10. 一次完整的`Http`请求是什么样子？
11. 对称加密和非对称加密？
12. `SSL` 的工作方式了解过么？
13. `Session`、 `Cookie`、`Token`三者之间的区别是什么？
13. 什么是 `Socket` ？
...
1. 有没有想过为什么会有集合框架的存在？
2. 集合有哪些特点？
3. 谈谈你对数据结构的理解，比如连续的内存结构和不连续的内存结构？
4. 谈谈你对 `Util` 集合框架的理解？都有哪些接口和类？
5. 集合底层的数据结构是什么样子的？
6. `List` 和 `Set` 之间有什么区别？
7. `ArrayList` 与 `LinkedList` 之间的区别?
8. `ArrayList` 的插入删除性能一定会比 `LinkedList` 慢么？
9. 谈谈 `HashMap` 的原理？
10. 说一下`HashMap`的几个参数：负载因子、初试容量？
11. 为什么不之间使用 `HashCode` 作为 table 下标？
12. `HashMap` 的 扩容流程？
13. `HashMap`中的链表转化红黑树需要哪两个条件？
14. 关于 `TreeMap` 的理解说一下？新元素插入需要提起准备什么？
15. 解决哈希冲入都有哪些方式呀？（四种）
16. `HashTable` 中 `key` `value` 为什么不能为`null`，而 `HashMap` 中却可以？
17. `HashTable`与`HashMap`之间的区别是什么？
18. 什么是迭代器 ？为什么要使用迭代器？
19. 使用迭代器的好处是什么？
20. 关于`FastFail`机制是什么？它有什么作用 ？
21. `CopyOnWriteArrayList` 的具体实现原理 ？
22. `ConcurrentHashMap` 的具体实现原理 ？
...
13. 常用的线程操作方法 `sleep` 、`wait`、`notify`、`notifyAll`，简单聊一下他们的使用？
14. 关于 `sleep() `方法 与 `yeild()` 方法有什么区别？
15. 如何停止一个正在运行的线程？`interrupted` 和 `stop`
16. 如何实现多线程之间的通讯和协作（两种方案）？
17. 线程死锁的理解？
18. 线程死锁的解决方案？
19. 什么是线程安全？
20. 如何保证线程安全？
21. 线程过多会导致什么问题？
22. synchronized 与 lock 区别？
23. 什么是锁消除？
24. 什么是锁的自旋？
25. 什么是锁的升级？
26. 谈谈你对 `AQS` 的理解吧？主要是它的一个实现原理？
27. Atomic 的运行原理 ？`CAS` 原理是什么？ 
28. `ABA`问题？
29. 如何处理`ABA`问题？`AtomicStampedReference `
30. 并发工具类Semaphore、`CountDownLatch`、`CyclicBarrier` 之间的区别？
31. 线程 B 怎么知道线程 A 修改了变量？ 线程之间的通信 （好多种方案）
    1.  volatile
    2. static
32. `volatile` 实现多线程的可见性？
33. 线程池的 7 大参数？核心线程可以被销毁么？
34. `submit`和`excute`有什么区别？
35. 关闭线程池以及马上关闭线程池的方法是什么？它们之间有什么区别？
    1. `Showdown()`
    2. `Terminated()`
    3. `ShutdownNow()`

36. 线程池的拒绝策略有哪四种？
37. 新任务到线程池以后，它的运行流程怎么样？
38. `Future`、`FutureTask` 与 `Completefutere` 的区别？
39. 并发工具类？
    1. `CountDownLatch`
    2. `CyclicBarrier`
    3. `Semaphore`
...
40. `int++`是线程安全的么？
41. `ThreadLocal`的使用场景？
42. 关于 `ThreadLocal` 可能引发的内存泄漏的问题，原因是什么？
43. `ForkJoin` 有用过么？思想是什么？
...
8. 常量池中的符号引用为什么存在？
9. 常量池的三种分类？
10. 字符串常量池为什么会被放在堆空间中？
11. 方法区的`GC`机制有什么特点？
12. 为什么叫 `HotSpot`？
13. 谈谈对象的创建过程 ？
14. 对象头里面都存放了哪些信息？类信息、对象年龄、锁标志位、哈希信息等
15. 对象访问的两种方法？
16. 内存泄漏和内存溢出的区别？
17. 内存溢出的解决方案？
18. 根可达的位置有哪些？4种
...
22. 简述一下分代算法？
23. `CMS` 工作流程是什么样子的？
24. 什么是三色标记法？（代表对象被清理的过程）
25. `STW` 一般发生在程序的什么时候 ？
26. `G1` 的算法原理是什么？相比分代算法的优势？对比`CMS`又有什么样的特点？
27. 了解过 `ZGC` 么？
28. 什么是GC的吞吐量？
29. 什么是GC的低延迟？
30. 在配置垃圾回收器的时候，场景该如何配置？
31. Java为什么解释与编译并行？
32. 什么时候，会判断当前代码是热点方法？
33. 谈谈你对类加载机制的了解？
34. 有哪些类加载器？
35. 为什么是双亲委派机制？类加载的流程？
36. 为什么要自定义类加载器？
37. 如何自定义类加载？
38. 什么时候虚拟机会退出？
39. 为什么要进行 JVM 调优？
