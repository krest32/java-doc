...
1. 听过监听器实现注册
2. Nacos继承了某个抽象类（继承了监听器），通过监听器发布事件，来注册服务

心跳检测

1. 心跳检测是开启了一个新的线程执行检测任务
2. 
...
![image-20220518234339950](img/image-20220518234339950.png)
...
public class NacosDiscoveryAutoConfiguration {
    ...
    public NacosServiceRegistry nacosServiceRegistry(NacosDiscoveryProperties nacosDiscoveryProperties) {
        // 读取 application.yml 中 Nacos 的配置
...
![image-20210907213647145](img/73c8d67736863d4dbda3549726b40af6.png)



AbstractAutoServiceRegistration

![![image-20210907214411267](https://img-blog.csdnimg.cn/img_convert/c8272093633621d68a87351ba22fcda1.png)](img/b87b819803d1b8304adb54d2caa044e1.png)可以看到它实现了`ApplicationListener`接口，监听Spring容器启动过程中的事件。

在监听到`WebServerInitializedEvent`（web服务初始化完成）的事件后，执行了`bind` 方法。

![image-20210907214411267](img/c8272093633621d68a87351ba22fcda1.png)
...
![image-20210907215903335](img/90047920a6af6b9bad5c3250f62243a4.png)
...
![image-20210923185331470](img/825542c097155504ec7c5e5fd8cd69aa.png)
...
![image-20220518105318381](img/image-20220518105318381.png)
...
![image-20210922141703128](img/f50a4b9f1c23ea7e42e22615dcd5f3af.png)
...
![img](img/31cac3e7293d4e19fb102f5919fe5ff9.png)
...
notifier的类型就是线程，内部维护了一个阻塞队列，存放服务列表变更的事件：

![image-20210922180246555](img/8e4fd4c23e2599eeed915660b480bff5.png)
...
![image-20210922195603450](img/e604c827802132e405c7434388eb700e.png)
...
![image-20210923214042926](img/aec23298168b06456750edde9beaf564.png)
...
服务注册这一节中，我们说过NacosNamingService这个类实现了服务的注册，同时也实现了服务心跳：
...
![image-20210922213313677](img/3501ed8cafd075b1caf8a15324b7f7fc.png)

### BeatReactor

而`BeatReactor`这个类则维护了一个线程池：

![image-20210922213455549](img/ee0d07cd640145f85d1da14b909ac855.png)
...
![image-20210922213829632](img/63372ee805c6b9a584174ad7b238539a.png)



### BeatTask

跳的任务封装在`BeatTask`这个类中，是一个Runnable，其run方法如下：
...
![img](img/c289669945273ddf4b85021593d4e343.png)
...
![image-20210922221344417](img/c63ca758501e7fbdcc60278188a349e8.png)

### 主动健康检测

对于非临时实例（eph

emeral=false)，Nacos会采用主动的健康检测，定时向实例发送请求，根据响应来判断实例健康状态。

入口在2.3.2小节的`ServiceManager`类中的registerInstance方法：

![image-20210923100740065](img/feccfdda75ec98eed70d69b503cc6461.png)
...
![image-20210923102824451](img/b440af7918401c567776878709700eec.png)
...
我们讲到一个类`NacosNamingService`，这个类不仅仅提供了服务注册功能，同样提供了服务发现的功能。

![image-20210923153419392](img/c979cdb384a0109aa02ec10dfcc3725e.png)
...
订阅服务消息，这里是由`HostReactor`类的`getServiceInfo()`方法来实现的：
...
![image-20210923161528710](img/430defc7432350aea040162c36d7aa42.png)

+ 如果本地缓存有，则开启定时更新功能，并返回缓存结果：
  + `scheduleUpdateIfAbsent(serviceName, clusters)`

![image-20210923161630575](img/1a71ae437a004863e609a75caca3d873.png)



在UpdateTask中，最终还是调用updateService方法：

![img](img/a2c94b718c6a3e1b4e414f44ea9d1c3f.png)
...
![image-20210923164145915](img/eb7697161777ad27fb182b7cf3778355.png)
...
介绍的InstanceController中，提供了拉取服务列表的接口：
...
![image-20210923182429636](img/d35dfacaf6c178b6e74ab4638cc712ae.png)
...
![image-20210923183017424](img/f02f4135485d6b145fd496d87a534114.png)
